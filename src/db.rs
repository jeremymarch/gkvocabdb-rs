...
pub async fn get_seq_by_prefix(pool: &AnyPool, table:&str, prefix:&str) -> Result<i32, sqlx::Error> {
  ...
  let rec:Result<(i32,), sqlx::Error> = sqlx::query_as(&query)
  .fetch_one(pool)
  .await;
  ...
  match rec {
      ...
      Err(sqlx::Error::RowNotFound) => { //not found, return seq of last word
          ...
          let max_rec:(i32,) = sqlx::query_as(&max_query)  //fake it by loading it into DefRow for now
          .fetch_one(pool)
          .await?;
...
pub async fn load_lemmatizer(db: &AnyPool) {
...
pub async fn get_lemmatizer(db: &AnyPool) -> HashMap<String, i32> {
    ...
    let res: Result<Vec<LemmatizerRecord>, sqlx::Error> = sqlx::query(query)
        .map(|rec: AnyRow| LemmatizerRecord {
            form: rec.get("form"),
...
pub async fn get_hqvocab_column(
    ...
    let words: Vec<(String, i32, String)> = sqlx::query_as(&query).fetch_all(pool).await?;
...
pub async fn insert_gloss(
    ...
    if let Some(new_gloss_id) = res.last_insert_id() {
        update_log_trx(
            &mut tx,
            UpdateType::NewGloss,
            Some(new_gloss_id),
...
/*
pub async fn fix_assignments(pool: &AnyPool) -> Result<(), sqlx::Error> {
  let mut tx = pool.begin().await?;
/*
...
*/
let mut text_order:i32 = 35;

  let query = "SELECT title,start,end,id FROM assignments WHERE id > 27 and id < 116;";
  let assignments:Vec<(String,i32,i32,i32,)> = sqlx::query_as(query)
...
*/
/*
pub async fn get_parent_text_id(pool: &AnyPool, text_id:i32) -> Result<Option<i32>, sqlx::Error> {
  let query = "SELECT parent_id FROM texts WHERE text_id = ?;";
  let rec: (Option<i32>,) = sqlx::query_as(query)
  .bind(text_id)
  .fetch_one(pool)
  .await?;
...
pub async fn get_text_name(pool: &AnyPool, text_id: i32) -> Result<String, sqlx::Error> {
...
//     let gloss_ids: Vec<(i32,)> = sqlx::query_as(&query)
//         .bind(text_id)
//         .fetch_all(tx)
//         .await?;
...
/*
pub async fn _get_titles(pool: &AnyPool) -> Result<Vec<(String,i32)>, sqlx::Error> {
    let query = "SELECT id,title FROM titles ORDER BY title;";
    let res: Result<Vec<(String,i32)>, sqlx::Error> = sqlx::query(query)
    .map(|rec: AnyRow| (rec.get("id"),rec.get("title")) )
    .fetch_all(pool)
    .await;
...
/*
pub async fn old_get_text_id_for_word_id(pool: &AnyPool, word_id:i32) -> Result<i32, sqlx::Error> {
...
/*
pub async fn get_start_end(pool: &AnyPool, text_id:i32) -> Result<(i32,i32), sqlx::Error> {
  ...
  let rec: (i32,i32) = sqlx::query_as(query)
  .bind(text_id)
  .fetch_one(pool)
  .await?;
...
pub async fn get_gloss_occurrences(
    ...
    let mut res: Vec<GlossOccurrence> = sqlx::query(&query)
        ...
        .bind(gloss_id)
        .bind(gloss_id)
        .map(|rec: AnyRow| GlossOccurrence {
            name: rec.get("name"),
...
pub async fn get_update_log(
    ...
    let res: Vec<(String, String, String, String, i32)> = sqlx::query(&query)
        .map(|rec: AnyRow| {
            (
                rec.get("timestamp"),
                rec.get("update_type"),
...
pub async fn get_before(
    ...
    page: i32,
    limit: i32,
) -> Result<Vec<(String, i32, String, i32)>, sqlx::Error> {
    let course_id = 1;
    let query = format!("WITH gloss_total AS (
        SELECT gloss_id, COUNT(gloss_id) AS total_count
        FROM words a2
    ...
    SELECT a.gloss_id,a.lemma,a.def,b.total_count FROM glosses a LEFT JOIN gloss_total b ON a.gloss_id=b.gloss_id WHERE a.sortalpha COLLATE PolytonicGreek < '{}' and status > 0 and pos != 'gloss' ORDER BY a.sortalpha COLLATE PolytonicGreek DESC LIMIT {},{};", course_id, searchprefix, -page * limit, limit);
    let res: Result<Vec<(String, i32, String, i32)>, sqlx::Error> = sqlx::query(&query)
        .map(|rec: AnyRow| {
            (
                rec.get("lemma"),
                rec.get("gloss_id"),
...
pub async fn get_equal_and_after(
    ...
    SELECT a.gloss_id,a.lemma,a.def,b.total_count FROM glosses a LEFT JOIN gloss_total b ON a.gloss_id=b.gloss_id WHERE a.sortalpha COLLATE PolytonicGreek >= '{}' and status > 0 and pos != 'gloss' ORDER BY a.sortalpha COLLATE PolytonicGreek LIMIT {},{};", course_id, searchprefix, page * limit, limit);
    let res: Result<Vec<(String, i32, String, i32)>, sqlx::Error> = sqlx::query(&query)
        .map(|rec: AnyRow| {
            (
                rec.get("lemma"),
