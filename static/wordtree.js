...
let platform;
if (window.navigator.userAgent.indexOf('iPhone') !== -1) platform = 'iphone';
else if (window.navigator.userAgent.indexOf('iPad') !== -1) platform = 'ipad';
else if (window.navigator.platform.toLowerCase().indexOf('mac') !== -1) { platform = 'mac'; } else if (window.navigator.platform.toLowerCase().indexOf('linux') !== -1) { platform = 'linux'; } else platform = 'windows';

function lookupWT (idPrefix) {
  for (let i = 0; i < globalWordTrees.length; i++) { if (globalWordTrees[i][0] === idPrefix) return globalWordTrees[i][1]; }
...
function Wordtree (idPrefix, width, height) {
  ...
  ) { this.autofocus = false; } else this.autofocus = true;
  ...
  this.closedNodeSVG =
        '<svg class="nodeImg nodePlus" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M24 10h-10v-10h-4v10h-10v4h10v10h4v-10h10z"/></svg>';
  this.openNodeSVG =
        '<svg class="nodeImg nodeMinus" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M0 10h24v4h-24z"/></svg>';

  input.onkeydown = wordtreeOndown;
  input.onkeyup = wordtreeOnup;

  if (this.showMorph && morph !== null) {
  ...
  function setWidth (width) {
    ...
    if (this.entry) this.entry.style.width = width - 76 + 'px';
  ...
  function conOnScroll (e) {
    ...
    if (
      wt.con.scrollTop >
                wt.con.scrollHeight - wt.con.offsetHeight - whenToGetNextPage &&
            !wt.nextPageRequestPending &&
            !wt.lastPage &&
            !wt.blockScroll
    ) {
      ...
      wt.mode === 'context' &&
            wt.con.scrollTop < whenToGetNextPage &&
            !wt.prevPageRequestPending &&
            !wt.lastPageUp &&
            !wt.blockScroll
    ) {
  ...
  function onMouseWheel (e) {
    ...
    if (browser === 'ie') {
      ...
      let match = /(.*)Container/.exec(el.id);
      if (!match) {
        match = /[0-9]+(.*)(Word)|(Root)/.exec(el.id); // FIXIDREF
        if (match) el = document.getElementById(match[1] + 'Container');
        else return;
      }
    } else {
      el = this;
    ...
    const amount = e.detail
      ? e.detail * mouseWheelSpeedFactor
      : (e.wheelDelta / 40) * mouseWheelSpeedFactor * -1;
    ...
    */
    ...
    if (key === 40 || key === 38) {
      if (typeof wt.onSelectionChanged === 'function' && wt.selectedRow) {
        wt.onSelectionChanged(
          wt.params.lexicon,
          getColumnValues(wt.selectedRow)
        );
      }
    } else if (
      !ev.ctrlKey &&
            ((key >= 48 && key <= 90) || key === 8 || key === 46 || key === 0) &&
            wt.asYouType
    ) {
      ...
      wt.lastKeyTimeout = setTimeout(
        "lookupWT('" + wt.idPrefix + "').refresh();",
        keyDelay
      );
    } else if (key === 27) {
      // esc
  ...
  function wordtreeOndown (ev) {
      ...
      if (wt.scrollTimer) {
        if (!wt.scrollTimerKeyDown) {
          if (!wt.scrollTimerTimeout && wt.step > 1) {
            wt.scrollTimerTimeout = setTimeout(function () {
              wt.scrollTimerKeyDown = true;
              move(1, wt, wt.step);
            }, wt.scrollTimerDelay);
          }
        ...
        if (!wt.downkey) {
          // eslint-disable-next-line no-implied-eval
          wt.accelTimeout = setTimeout(
            "var a = lookupWT('" +
                            wt.idPrefix +
                            "'); a.step = 2; a.accelTimeout = setTimeout(\"lookupWT('test1').step = keyScrollAccel\", 2000)",
            2000
          );
        }
      }
    ...
    if (!wt.downkey) {
      ...
      if (ev.ctrlKey) {
        ...
        if (key === 68) {
          // d
          if (
            typeof wt.onDeleteWord === 'function' &&
                        wt.selectedRow
          ) {
            wt.onDeleteWord(
              getColumnValues(wt.selectedRow)[1],
              getColumnValues(wt.selectedRow)[2],
              wt.params.tag_id
            );
        ...
        case 27: // esc
          ...
          if (wt.refreshOnESC) {
            ...
            wt.lastKeyTimeout = setTimeout(
              "var a = lookupWT('" +
                                wt.idPrefix +
                                "'); a.refresh(); if (a.entry && a.autofocus) a.entry.focus()",
              keyDelay
            );
          }
          break;
    ...
    if (
      !ev.ctrlKey &&
            !ev.metaKey &&
            (wt.params.lexicon === 'lsj' || wt.params.lexicon === 'slater')
    ) {
      return true; // transliterateKey(ev);
    } else {
      return true;
    }
  }
  ...
  function move (upDown, wt, step) {
    ...
    while (temp.id.indexOf('Container') === -1) {
      // this means we can't use "Container" as a prefix name
    ...
    // scroll as you go up
    if (childOffset < con.scrollTop + 30) {
      // was 15
      con.scrollTop = childOffset - 30; // was 18
    }

    // scroll as you go down
    if (childOffset > con.scrollTop + con.offsetHeight - 50) {
      // was 30
    ...
    if (wt.scrollTimer && wt.scrollTimerKeyDown) {
      setTimeout(function () {
        move(upDown, wt, wt.scrollTimerStep);
      }, wt.scrollTimerRate);
    }
  }
...
*/
  ...
  function procResponse (str, status) {
    ...
    if (wt.cache && returnObj.nocache === 0) {
      // the only one we don't cache is if if it's looked up via wordid
    ...
    if (returnObj.parentid) {
      // && returnObj.treeOptions.length > 0)
      for (let i = 0; i < len; i++) {
      ...
      if (wt && returnObj.scroll === 'top') {
        ...
        if (s) {
          if (wt.selectedRow) {
            if (
              wt.selectedRow.classList.contains(
                'selectedRowClass'
              )
            ) {
              wt.selectedRow.classList.remove('selectedRowClass');
      ...
      //   node.style.color = 'red';
      //   node.style.fontWeight = 'bold';
      //   if (returnedPage < 0 && before) con.insertBefore(node, before);
      //   else con.appendChild(node);
      // }
      ...
      if (
        wt.selectedRow &&
                returnedPage === 0 &&
                typeof wt.onSelectionChanged === 'function'
      ) {
        wt.onSelectionChanged(
          wt.params.lexicon,
          getColumnValues(wt.selectedRow)
    ...
    function printRow (
      ...
      // for each column
      const rowLen = rowItem.col.length;
      for (let c = 0; c < rowLen; c++) {
        ...
        if (level === 0) {
          d2.style.paddingLeft =
                        1 + (!rowItem.h && topLevelTreeRow ? 15 : 3) + 'px';
        } else {
          d2.style.paddingLeft =
                        level * wt.indentWidth + (rowItem.h ? 3 : 15) + 'px';
        }

        if (c === 0 && rowItem.h) {
          // only for column 1, if has child nodes
      ...
      if (insertAtTop && before) con.insertBefore(node, before);
      else con.appendChild(node);
    }
  ...
  function toggleNode (wt, nodeid) {
    ...
    if (!imgNode) {
    // its ok for n to be null (lazy load)
      return;
    }
  ...
  function openNode (wt, nodeid) {
    ...
    if (!imgNode) {
    // its ok for n to be null, but not imgNode
      return true;
    }

    if (imgNode.classList.contains('nodeOpen')) return false;
    ...
    if (n) {
      ...
      if (imgNode.parentNode.parentNode.nextSibling) {
        wt.con.insertBefore(
          childrenCon,
          imgNode.parentNode.parentNode.nextSibling
        );
      } else wt.con.appendChild(childrenCon);

      wt.params.root_id = getColumnValues(
  ...
  function checkCache (wt) {
    ...
    let queryKey;
    if (wt.entry.value === '') {
      queryKey = wt.params.lexicon + 'a' + wt.params.tag_id;
    } /* because wtgreekserv changes "" to "a" */ else {
      queryKey =
                wt.params.lexicon + wt.entry.value + wt.params.tag_id;
    }
  ...
  function wtAddResultToCache (wt, queryKey, str) {
    ...
    if (!wt.cache[queryKey]) {
      ...
      if (
        queryKey === wt.params.lexicon + 'a' ||
                queryKey === wt.params.lexicon + 'α'
      ) {
        // put empty queries into the future, so they are never removed from cache
        const d = new Date();
      ...
      // don't reset timestamp for empty query
  ...
  function centerSelectedRow () {
    if (this.selectedRow) {
      // scroll to middle
      this.con.scrollTop =
                this.selectedRow.offsetTop - this.con.offsetHeight / 2 + 30;
    }
  }
}
...
function getColumnValues (row) {
  ...
  for (let i = 0; col; i++) {
    const match = /.*Img/.exec(col.id); // in case it has a + or - image
    if (match) {
